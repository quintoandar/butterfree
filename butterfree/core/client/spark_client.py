...
class SparkClient:
    ...
    def read_table(self, database, table):
        ...
        if not isinstance(database, str):
            raise ValueError("database needs to be a string with the name of the metastore schema")
        if not isinstance(table, dict):
            raise ValueError("table to be a string with the name of the registered table")
