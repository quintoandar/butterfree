...
from pyspark.sql import DataFrame, functions, WindowSpec
...
class AggregatedFeatureSet(FeatureSet):
    ...
    @staticmethod
    def _build_feature_column_name(feature_column: str, pivot_value: str = None, window: Window = None):
    ...
    def with_pivot(self, column: str, values: List[str]) -> "AggregatedFeatureSet":
    ...
    def _get_base_dataframe(self, client: SparkClient, dataframe: DataFrame, end_date: str) -> DataFrame:
    ...
    @staticmethod
    def _dataframe_join(left: DataFrame, right: DataFrame, on: List[str], how: str, num_processors: int = None) -> DataFrame:
    ...
    def _aggregate(self, dataframe: DataFrame, features: List[Feature], window: Window = None, num_processors: int = None):
        aggregations = [
    ...
    def _with_renamed_columns(self, aggregated: DataFrame, features: List[Feature], window: Window):
        old_columns = [
            c
    ...
    def construct(
        ...
        output_df = output_df.select(*self.columns).replace(float("nan"), None) #type: ignore
