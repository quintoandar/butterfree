...
class TestForwardFillTransform:
    def test_forward_fill_transform(self, input_df):
        ...
        # assert
        assert all(
            [r.has_feature == 1 for r in output_df.filter("pivot_column = 3").collect()]
        )
    ...
    def test_forward_fill_transform_id_partition(self, input_df):
        ...
        # assert
        assert (
            output_df.filter("pivot_column = 3").orderBy("ts").collect()[-1].has_feature
            == 0
        )
    ...
    def test_forward_fill_transform_new_column(self, input_df):
        ...
        # assert
        assert "has_feature_filled" in output_df.columns
        assert output_df.filter("has_feature_filled is null").count() == 0
